...
public class VideoEncoder {
    ...
    private final List<Bitmap> srcBitmaps;
    private final int count;

    public VideoEncoder(List<Bitmap> srcBitmaps, int width, int height, double time, int count, File outFile) {
        this.srcBitmaps = srcBitmaps;
        mWidth = width;
        mHeight = height;
            ...
            for (int i = 0 ; i < count ; i++) {
                for (Bitmap image : srcBitmaps) {
                    ...
                    if (image.getHeight() == mHeight && image.getWidth() == mWidth) {
                        ...
                        modImage = Util.getResizedBitmap(image, mWidth, mHeight, modImage);
                        bmToUse = modImage;
                    }
                    for (double fm = 0.1 ; fm <= time ; fm += 0.05) {
                        ...
                        if (VERBOSE) Log.d(TAG, "sending frame " + frameNum + " to encoder");
    ...
    private void prepareEncoder() throws IOException {
        ...
        format.setFloat(MediaFormat.KEY_FRAME_RATE, FRAME_RATE);
    ...
    private void generateSurfaceFrame(Bitmap bitmap) {
        ...
        Util.renderTexture(texture, mWidth, mHeight);
    }
